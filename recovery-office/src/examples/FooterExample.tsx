...
const FooterExample: React.FC = () => {
  ...
  const Logo = () => (
    <Box mb={5}>
      <Text as="h2" variant="h3" style={{ color: 'white' }}>
        Recovery Office
      </Text>
      <Box mt={2}>
  ...
  return (
      ...
      bottomContent={
        <Box display="flex" flexDirection="row">
          {legalLinks.map((link, index) => (
            <Box 
              key={`legal-${index}`} 
              mr={index < legalLinks.length - 1 ? 3 : 0}
            >
              <Text 
                variant="body2" 
    ...
    >
      {/* First column - About */}
      <Box>
        <Text variant="body2" style={{ marginBottom: '16px' }}>
      ...
      <FooterLinks
        title="Resources"
        links={resourceLinks}
        description="Explore our healing resources and educational content"
        spacing="md"
        withLinkDecoration={true}
        linkIcon={
          <OliveBranch
      ...
      <FooterNewsletter
        ...
        description="Subscribe to receive updates on new healing methodologies and events."
        placeholder="Your email address"
        buttonText="Subscribe"
        onSubmit={handleNewsletterSubmit}
        successMessage="Thank you! You're now subscribed to our newsletter."
      >
        <Text variant="body2" style={{ opacity: 0.6, fontSize: '0.8rem' }}>
