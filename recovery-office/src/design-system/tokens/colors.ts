...
export const colorUtils = {
   ...
   */
  lighten: (color: string, intensity: 'low' | 'medium' | 'high' = 'medium'): string => {
   ...
   */
  darken: (color: string, intensity: 'low' | 'medium' | 'high' = 'medium'): string => {
    const factors = {
      low: PHI_INVERSE * 0.5,
      medium: PHI_INVERSE,
   ...
   */
  withAlpha: (color: string, level: keyof typeof BASE_COLORS.green): string => {
