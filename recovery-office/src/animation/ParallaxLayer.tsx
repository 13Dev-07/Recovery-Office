...
export const ParallaxLayer = forwardRef<HTMLDivElement, ParallaxLayerProps>(
  ({ 
    ...
    const calculateYTransform = (scrollProgress: MotionValue<number>) => {
      if (!isEffectivelyEnabled) return 0;
    ...
    const transformValue = calculateYTransform(scrollProgress);
    
    // Apply transforms based on direction
    const transformX = horizontal ? transformValue : 0;
    const transformY = horizontal ? 0 : transformValue;
