 ...
 */
var getFibonacciValue = function (index) {
    ...
    var key = fibKeys[safeIndex];
    var value = sacred_geometry_1.FIBONACCI[key];
    if (typeof value !== 'number') {
        throw new Error("Invalid Fibonacci index: ".concat(index, ". No value found for key: ").concat(key, "."));
    }
    return value;
};
...
var fibonacciDuration = function (index) {
    ...
    var fibValue = getFibonacciValue(index);
...
var monitorAnimationPerformance = function (callback, options) {
    ...
    var name = options.name, _a = options.threshold, threshold = _a === void 0 ? getFibonacciValue(7) : _a, // 13ms (close to 16ms frame budget)
