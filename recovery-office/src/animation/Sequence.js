 ...
 */
exports.Sequence = (0, react_1.forwardRef)(function (_a, ref) {
    ...
    var _p = React.useState(false), hasMounted = _p[0], setHasMounted = _p[1];
    ...
    var childVariants = getVariants(variant, distance, duration, easingFunction);
    ...
    var processedChildren = react_1.Children.map(children, function (child, index) {
        ...
        var customVariants = __assign(__assign({}, childVariants), { visible: __assign(__assign({}, childVariants.visible), { transition: childVariants.visible && 'transition' in childVariants.visible
                    ? __assign(__assign({}, childVariants.visible.transition), { delay: delay }) : { delay: delay } }) });
    ...
    return (<framer_motion_1.motion.div ref={ref} variants={containerVariants} initial="hidden" animate={isVisible && shouldAnimate ? "visible" : "hidden"} {...rest}>
...
var calculateStaggerDelay = function (index, total, baseDelay, useFibonacci, direction) {
    ...
    var fibValue = fibValues[fibonacciIndex] || 1;
    ...
    var fibSum = fibValues.slice(0, maxFibIndex + 1).reduce(function (sum, num) { return sum + num; }, 0) || 1;
...
var getVariants = function (variant, distance, duration, easing) {
    ...
    switch (variant) {
        ...
        case 'scale':
            return {
                hidden: { scale: sacred_geometry_1.PHI_INVERSE, opacity: 0 },
                visible: {
                    scale: 1,
                    opacity: 1,
                    transition: baseTransition
